...
把 `shinoyuki_betterautosave-0.10.0.jar` 丢进服务端的 `mods/` 文件夹，启动即可。第一次启动后，配置文件会生成在：
...
完整兼容性矩阵见 [ROADMAP](ROADMAP.md#bas-兼容性矩阵-代码核对)。
...
技术原理、生态调研、完整兼容性矩阵、版本路线图见 [ROADMAP.md](ROADMAP.md)。
